...
# Интеллектуальный выбор алгоритма на основе метрик
def choose_best_algorithm(data_type, file_size):
    if data_type == 'image':
        ...
        return 'zlib' if file_size < 1 * 1024 * 1024 else 'lzma'

# Функция выполнения эксперимента
def run_experiment(input_path, output_path, data_type):
    file_size = os.path.getsize(input_path)
    algorithm = choose_best_algorithm(data_type, file_size)
...
run_experiment('example.mp4', 'compressed.mp4', 'video')
run_experiment('example.wav', 'compressed.flac', 'audio')
